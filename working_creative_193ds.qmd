...
mtcars_figure <- function(seed, n, palette, seed_spatial, freq_spatial, fern_points) {
  ...
  dat <- tibble(
    ...
    shade = runif(n), 
    size = runif(n),
    paint = sample(spatial_data$paint, n)
  )
...
mtcars_seed <- mtcars_figure(seed = seed, n = n, palette = palette, seed_spatial = seed_spatial, freq_spatial = freq_spatial, fern_points = fern_points)
...
#FROM PART B - introductory exercises
pal <- c("#cdb4db", "#ffc8dd", "#ffafcc", "#bde0fe", "#a2d2ff")

#FROM PART C - spatial noise tricks
smol_simplex <- smol_grid |>
  mutate(z = gen_simplex(x, y, seed = 1, frequency = .1)) 

smol_simplex |>
  ggplot(aes(x, y, z = z)) + 
  geom_tile(aes(fill = z)) +  # Change to geom_tile() for more regular rasterization
